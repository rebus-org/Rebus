...
namespace Rebus.ReturnToSourceQueue.AzureServiceBus
{
    class Program
    {
        ...
        {
            return new NiceException(@"Rebus Return To Source Queue Tool - Azure Service Bus flavor    (-■_■)

Invoke without any arguments

    returnToSourceQueue.azuresb.exe

to be prompted for each option. Or invoke with the following arguments:

    returnToSourceQueue.azuresb.exe <errorQueueName> [--auto-move] [--dry]
...
e.g. like this:
    ...
    returnToSourceQueue.azuresb myErrorQueue

in order to start processing the messages from 'myErrorQueue', or

    returnToSourceQueue.azuresb myErrorQueue --auto-move

in order to automatically retry all messages that have the '{0}' header set, or

    returnToSourceQueue.azuresb myErrorQueue --auto-move --dry
...
in order to SIMULATE automatically processing all messages (queue transaction will be aborted).", Headers.SourceQueue);
        ...
        static bool AzureServiceBusErrorQueueExists(Parameters parameters)
        {
            var namespaceManager = NamespaceManager.CreateFromConnectionString(parameters.ConnectionString);
            return namespaceManager.QueueExists(parameters.ErrorQueueName);
        }
