...
namespace Rebus.RabbitMQ
{
    ...
    public class RabbitMqMessageQueue : IMulticastTransport, IDisposable, INeedInitializationBeforeStart
    {
        ...
        static ReceivedTransportMessage GetReceivedTransportMessage(IBasicProperties basicProperties, byte[] body)
        {
            ...
            // Populate rebus-msg-id from transport-level-id, if such id is available.
            if (basicProperties != null && !string.IsNullOrWhiteSpace(basicProperties.MessageId))
            {
                if (result.Headers.ContainsKey(Headers.MessageId))
                {
                    ...
                    var headerLevelId = Convert.ToString(result.Headers[Headers.MessageId]);
                    if (!string.Equals(headerLevelId, basicProperties.MessageId))
                    {
                        log.Warn("Missmatch between transport-level and header-level message's id. " +
                                 ...
                                 basicProperties.MessageId, headerLevelId);
                    }
                }
                result.Headers[Headers.MessageId] = basicProperties.MessageId;
            }
